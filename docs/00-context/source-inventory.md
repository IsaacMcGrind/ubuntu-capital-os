...
| SRC-012 | `contractors/HerLogicSolutions/azure-architecture.md` | Contractor architecture evidence | Supports the proposed Azure cloud architecture and platform-service composition for the Ubuntu Capital MVP workstream | CONFIRMED SUPPORT | Architecture proposal/supporting evidence only; it does not independently confer project-level approval or implementation completion. |
| SRC-013 | `contractors/HerLogicSolutions/azure-mvp-cost-breakdown.md` | Contractor cost/architecture evidence | Supports MVP Azure cost assumptions, low-cost service choices, and cost-governance direction | CONFIRMED SUPPORT | Cost/planning evidence only; actual Azure spend and provisioned tiers require runtime/account evidence. |

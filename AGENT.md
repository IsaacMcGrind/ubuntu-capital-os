...
1. Contractors and coders execute only the work package and path scope assigned by the Project Manager. A scope-level `GO` defines what may be assigned; it is not itself an executor assignment. No contractor may begin technical work until a durable Project Manager assignment identifies the contractor, exact work-package subset, authorised repository paths or bounded contractor subdirectory, evidence purpose, and applicable external implementation/resource scope.

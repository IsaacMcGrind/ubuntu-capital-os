...
15. Repository change policy: after required canonical outputs are present, all further updates must modify existing files only. Do not create new files unless explicitly approved by the project owner for a specific purpose. `SRC-021` explicitly approves the creation and continuing use of `.github/pull_request_template.md` as the mandatory PR-governance control. It also permits a new evidence file inside an existing contractor folder only when the contractor self-selection declaration identifies its exact path or bounded contractor subdirectory and evidence purpose; neither approval creates a general new-file or canonical-artifact exception.

## Delivery authority and contractor evidence boundary

Under `SRC-021`, 80K Developers (Pty) Ltd is the appointed Project Manager contractor and accountable project-management workstream. Thembinkosi Mtsweni is the responsible human Project Manager and final project-owner decision authority working through that management arrangement. In that capacity, Thembinkosi Mtsweni holds the sole repository authority for interpreting or accepting contractor evidence and changing canonical Ubuntu Capital OS delivery status, priorities, gates, or project boundaries. This authority does not include allocating contractor work; contractors self-select.

Within this Ubuntu Capital OS repository:

1. Contractors and coders may self-select any work. Each contractor documents its selected work-package/use-case scope, repository paths changed or bounded contractor subdirectory, evidence purpose, and applicable external implementation/resource scope in its PR. No assignment or Project Manager pre-start record is required. Self-selection is not evidence acceptance, status promotion, completion, architecture adoption, or permission to cross established project boundaries.
...
4. A contractor evidence submission must not create or reinterpret a project gate, change canonical project or work-package status, declare a use case or work package complete, make an architecture/business/legal decision for Ubuntu Capital, broaden the repository's established project boundaries, or present unsupported implementation claims as current fact.
5. Contractor evidence is an input to governance, not a governance decision. Merging contractor evidence does not promote status or alter project boundaries.
...
9. Every PR must complete the project-owner-approved `.github/pull_request_template.md` (`SRC-021`) with submitting organization, declared capacity, responsible human, work-package IDs, decision/context reference, repository paths changed, evidence references, validation, what the evidence proves/does not prove, and a sensitive-information check. Missing or unverifiable authority metadata makes the PR `Unverified` or `Changes required`.
...
`Contractor self-selects and begins any work inside established project boundaries without assignment -> contractor records sanitised evidence in its contractor folder -> contractor's PR declares the work selected, paths changed and evidence purpose -> evidence PR is reviewed -> evidence PR is merged as provenance only -> 80K Developers Project Manager workstream separately assesses the merged evidence -> accepted evidence is reconciled through a PROJECT_MANAGER_GOVERNANCE change -> Thembinkosi Mtsweni decides canonical status, priority, gates, and project-boundary changes`.
